...
**A1 落到入库侧**——也就是上面那张表揭示的真正根因。修了它，这次的
`declared_total_mismatch` 会直接消失，而不是降级成一条警告。没有在本轮一起
改，是因为它会改变**官方路径**上 `line_count` / 明细合计 的数字（89 而不是
90），属于需要单独确认的变更，不该夹在一次"让预览别卡住"的改动里。

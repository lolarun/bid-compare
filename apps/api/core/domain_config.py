...
# 判定"这一段是副本"的序列相似度下限。定得比 BLOCK_QTY_SIMILARITY_MIN(0.70) 更严——
# 那里比对的是数量这种粗粒度信号，这里比对的是 (name,spec,unit,qty) 整行内容，
# 信息量大得多；且这里的误判后果更重——把正常清单腰斩成两份，比漏判（行数偏多，
# 用户在疑点收件箱能看到）严重得多，宁可保守。
#
# 这个值**同时**给 copy_detect.py 两级判据把关，不是只管细粒度那一级：粗粒度
# （按 name 类目值）判据复用同一个阈值。浦东电缆实测两端都留了余量——真实副本的
# 类目序列相似度 0.9747，两段真正不同类目的对照组是 0.0——0.90 对两级都安全，
# 以后调这个值前两级都要重新核一遍，不能只按整行场景的直觉调。
COPY_ROW_SIMILARITY_MIN: float = 0.90

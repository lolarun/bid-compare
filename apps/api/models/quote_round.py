...
class QuoteRound(Base):
    ...
    seq: Mapped[int] = mapped_column(Integer, nullable=False)               # 1, 2, 3 … within (project_id, category)
    name: Mapped[str] = mapped_column(String(200), nullable=False, default="")  # user-authored label, e.g. "第一轮"

    stage: Mapped[str] = mapped_column(String(20), nullable=False, default=STAGE_FORMAL)
    ...
    is_final_basis: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    confirmed_supplier_ids: Mapped[Any] = mapped_column(JSON, nullable=True)
    used_submission_ids: Mapped[Any] = mapped_column(JSON, nullable=True)

    created_by: Mapped[str | None] = mapped_column(String(100), nullable=True)
    remark: Mapped[str] = mapped_column(Text, nullable=False, default="")

    opened_at: Mapped[datetime | None] = mapped_column(DateTime, default=_now)

...
> 最后更新：2026-08-22（新增识别准确率缺陷记录，用户明确暂不排期）；
> 结构性条目最后一次成批完成于 2026-06-23（§7.3/§10.3/§10.4/§11.2/§11.3/§11.4/§11.5，
> snapshot replay 7 失败修复）
...
## -1. 识别准确率（2026-08-22 记录，用户明确暂不排期）

用户当前优先级是**手工可用性测试**，不是提准确率。以下缺陷已定位到具体行，
不修，但必须记着——它们现在以「疑点」形式显示在预览里，不是消失了。

### -1.1 凯硕新正 PDF：4 行含税合价识别丢失（差 25,540 元 / 2.74%）

`job 12dc7d9af025…`（金桥地体上盖 · 阀门）。design/32 §9 的 A1 修掉「合计行
被当成报价行」之后，声明总价闭环从 97.899% 收敛到 **2.74%，仍超 0.5% 阈值**，
剩下的是真实识别缺陷：
...
这四行 `total_price_incl_tax` 为空，合计少 25,540 元。**门继续报警是对的**，
不要为了让它变绿去放宽阈值或补值。

### -1.2 同一份文件：1 行列串位导致数量丢失

`#89 缓闭式止回阀 DN100` —— 材质 `EPDM` 落进了 `unit` 列，`qty` 丢失，
金额 3,460 元是对的。

**连带后果（比缺陷本身更值得注意）**：该行照常入库（design/32 §9 的判据
特意保住了它，无数量不等于不是条目），但**进不了报价派生轴**——派生轴要用
数量序列校验对齐，没有数量就没法校验。于是出现「入库 89 条、矩阵 88 行」，
那 3,460 元在比价矩阵里看不见。

目前这个差额没有在界面上单独说明。修识别之前，至少应该让"有 N 条已入库但
未进入比价"这件事可见——**静默少一行比少一行更糟**。
（未做，跟 -1.1 一起排期。）
...
- [ ] **"离线"测试缺网络护栏**（复核意见，2026-08-13，design/27 §10 步骤1）
  `test_paddle_quote_api_e2e.py` 曾经在没 mock `text_call` 的情况下悄悄打了一次
  真实 DashScope API（这次是 checksum 门正确开火才暴露，不是测试本身报错）——
  这个事故模式值得记一笔：测试套件悄悄开始花钱/碰网，往往就是这么开始的。
  建议：给标记为"离线/回放"的测试加一层网络护栏（socket 级拦截，或 fixture
  显式断言"本次运行零外部调用"），让"忘了 mock"变成红灯而不是账单。不阻塞
  design/27 当前这轮，记在这里避免下次同样的坑悄悄发生第二次。
